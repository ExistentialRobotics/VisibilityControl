...
def pix2geom(pos, res=0.05, length=24):
    ...
    :param length: The length of the maps in meters.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mapSize', help='The size of the input maps', default='')
    ...
    for env_num in range(start, start + args.numEnv):
        ...
        for pathNum in range(args.numPaths):
            ...
            if data['success']:
                ...
                # # NOTE: Currently only valid for maps sizes of certain multiples.
                # predVal = model(encoder_input[None,:].float().cuda())
                # predClass = predVal[0, :, :].max(1)[1]
    ...
    if args.explore:
        fileName = osp.join(modelFolder,
                            f'eval_val{args.mapSize}_plan_exp_{args.segmentType}_{args.plannerType}_{start:06d}.p')
    else:
        fileName = osp.join(modelFolder,
                            f'eval_val{args.mapSize}_plan_{args.segmentType}_{args.plannerType}_{start:06d}.p')
